...
public class Slave implements Runnable {
	...
	public Slave(double wheelDiameter, double trackWidth, NXTRegulatedMotor leftMotor, NXTRegulatedMotor rightMotor) {
	...
	public void process(Curve c) {
		...
		double dTheta = DTheta(x, y, (double)c.GetXKth(0, 0), (double)c.GetYKth(0, 0));
		...
		for (int i=0; i<c.n-1; i++) {
			dTheta = DTheta(c.GetXKth(i, 0), c.GetYKth(i, 0), c.GetXKth(i, step), c.GetYKth(i, step));
